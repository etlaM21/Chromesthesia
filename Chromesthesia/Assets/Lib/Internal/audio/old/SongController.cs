	...
	public void getFullSpectrumThreaded() {
		try {
			...
			for (int i = 0; i < iterations; i++) {
				...
				/* START MY SHITTY ADDON */
				// Test Building Shit
				spectrumBoy.addToSpectrum(Array.ConvertAll (scaledFFTSpectrum, x => (float)x), curSongTime);
				//spectrumBoy.instSpectrumRow(Array.ConvertAll (scaledFFTSpectrum, x => (float)x), curSongTime);
				/* END MY SHITTY ADDON */
